# Escriba un programa que solicite las coordenadas de un punto tridimensional cartesiano de forma simbólica (solo símbolos a y b), por ejemplo, x = 3*a+b, y=7, z=a-b+5, y lo devuelva en coordenadas cilíndricas y esféricas. imprímalo en pantalla.

# Luego solicite los valores de a y b, lo reemplaza en el resultado simbólico anterior y muestre el resultado en forma numérica con 4 decimales.
...
x = sp.sympify(input("Ingrese la coordenada x en términos de 'a' y 'b' (ejemplo 3*a+b): "))
y = sp.sympify(input("Ingrese la coordenada y en términos de 'a' y 'b' (ejemplo 3*a+b): "))
z = sp.sympify(input("Ingrese la coordenada z en términos de 'a' y 'b' (ejemplo 3*a+b): "))
